...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (intent === "delete") {
    ...
    const { error } = await supabase.from("items").update({ title, description }).eq("id", params.id);
    if (error) {
      return { error: error.message };
    }
...
export default function Item({ loaderData, actionData }: Route.ComponentProps) {
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      {actionData?.updated && (
        <div className="bg-green-200 text-green-800 p-2 mb-4 rounded">Item updated successfully!</div>
      )}
